...
class LinearLayer(Layer):
    ...
    def __init__(self, n_in, n_out):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        self._W = xavier_init(n_in * n_out).reshape(n_in, n_out)
    ...
    def backward(self, grad_z):
        ...
        self._grad_b_current.resize((1, self.n_out))
...
            - neurons {list} -- Number of neurons in each linear layer
    ...
    def forward(self, x):
        ...
        # save all the layers when calling forward() the first time
        # after it, directly use the layer
        if len(self._layers) == 0:

            for i in range(len(self.neurons)):
                if i == 0:
                    linear = LinearLayer(self.input_dim, self.neurons[0])
                else:
                    linear = LinearLayer(self.neurons[i - 1], self.neurons[i])
                x = linear.forward(x)
                self._layers += linear,
...
class Trainer(object):
    ...
    @staticmethod
    def shuffle(input_dataset, target_dataset, random_generator=default_rng()):
        """
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for epoch in range(1, self.nb_epoch + 1):
            ...
            inputs = np.array_split(input_dataset, len(input_dataset) / self.batch_size)
            targets = np.array_split(target_dataset, len(target_dataset) / self.batch_size)

            loss_per_epoch = 0
            for batch_inputs, batch_targets in zip(inputs, targets):
                loss =  self.eval_loss(batch_inputs, batch_targets)
                gradient = self._loss_layer.backward()

                self.network.backward(gradient)
                self.network.update_params(self.learning_rate)
                loss_per_epoch += loss
            # print(f"loss(epoch {epoch}) {loss_per_epoch}")
            self.losses += loss_per_epoch,
...
class Preprocessor(object):
    ...
    def apply(self, data):
        ...
        data_normalised = (data - self.min) / (self.max - self.min)
    ...
    def revert(self, normalized_dataset):
        """
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        original_data = normalized_dataset * (self.max - self.min) + self.min
...
def example_main():
    ...
    # Q1.5
    original_dataset = prep_input.revert(prep_input.apply(x_train))
    # print(np.allclose(x_train, original_dataset))


if __name__ == "__main__":
    example_main()

    # l = LinearLayer(n_in=2, n_out=3)
    # x = np.array([[3, 8], [1, -1]])
    # outputs = l(x)
    # grad_loss_wrt_inputs = l.backward(outputs)
    # l.update_params(learning_rate=0.1)
    # print(f'W after update: {l._W}')
    # print(f'b after updates: {l._b}')

    # x = np.array([[3, 8], [1, -1]])
    # ls = SigmoidLayer()
    # outputs_s = ls.forward(x)
    # grad_loss_wrt_inputs_s = ls.backward(outputs_s)
    #
    # lr = ReluLayer()
    # outputs_r = lr.forward(x)
    # grad_loss_wrt_inputs_r = lr.backward(outputs_r)
    #
    # network = MultiLayerNetwork(input_dim=4, neurons=[16, 2], activations=["relu", "sigmoid"])
    # outputs = network(np.array([[1, 2, 3, 4]]))
    # grad_loss_wrt_inputs = network.backward(outputs)
